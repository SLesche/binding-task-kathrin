...
%%
% Nun werden die Stimuli in die Trials eingebaut
for i = 1: size(SampleMatrix,1)
    for j=expinfo.conditionInfo+1:expinfo.conditionInfo+SampleMatrix(i,3)%erste Spalte nach den Infos:so viele Spalten weiter, die die set size definiert ist für Trial i 
        test = 0;
        while test == 0
            SampleMatrix(i,expinfo.conditionInfo+1:expinfo.conditionInfo+SampleMatrix(i,3)) = randi([0 9],1, SampleMatrix(i,3)); % Stim 1
            if length(unique(SampleMatrix(i,expinfo.conditionInfo+1:expinfo.conditionInfo+SampleMatrix(i,3)))) == length(SampleMatrix(i,expinfo.conditionInfo+1:expinfo.conditionInfo+SampleMatrix(i,3)))
                test = 1;
            end
        end
    end
...
for trial = 1: size(TrialMatrix,1)
    ...
    Trial(trial).MemSet1 = TrialMatrix(trial,6);
    Trial(trial).MemSet2 = TrialMatrix(trial,7);
    Trial(trial).MemSet3 = TrialMatrix(trial,8);
    Trial(trial).MemSet4 = TrialMatrix(trial,9);
    Trial(trial).MemSet5 = TrialMatrix(trial,10);
    ...
    Trial(trial).MemSet = [Trial(trial).MemSet1 Trial(trial).MemSet2 Trial(trial).MemSet3 Trial(trial).MemSet4 Trial(trial).MemSet5];
    ...
    Trial(trial).MemSet(Trial(trial).MemSet<0) = [];
            ...
            if expinfo.Block == 2 %Updating differs depending on Condition
               test= 0;
                while test ==0
                    ...
                    if ~any(intersect(Trial(trial).Update(1:Trial(trial).UpdatingSteps), Trial(trial).MemSet)) && length(unique(Trial(trial).Update(1:Trial(trial).UpdatingSteps))) == Trial(trial).UpdatingSteps
                        test =1;
                    end
                end
            elseif expinfo.Block == 3
                test= 0;
                while test ==0
                    k= find(Trial(trial).MemPos == Trial(trial).UpdatePos(e));
                    Trial(trial).Update(e) = randi([0 9]);
                    if  Trial(trial).Operation(e) ==1 %Addition
                        if Trial(trial).MemSet(k) + Trial(trial).Update(e) <= 9
                            test =1;
                        end
                    elseif  Trial(trial).Operation(e) ==2 %Subtraktion
                        if Trial(trial).MemSet(k) - Trial(trial).Update(e) >=0
                            test =1;
                        end
                    end
                end
...
for trial = 1: length(Trial)
    ...
    for e = 1:Trial(trial).MemSetSize
        if expinfo.Block==1
            k = find(Trial(trial).MemPos== Trial(trial).ProbePos(e));
            Trial(trial).CorrResp(e)= Trial(trial).MemSet(k);
        end
        if expinfo.Block==2
            if ismember(Trial(trial).ProbePos(e), Trial(trial).UpdatePos)
                k = find(Trial(trial).UpdatePos== Trial(trial).ProbePos(e));
                Trial(trial).CorrResp(e)= Trial(trial).Update(k);
            else
                k = find(Trial(trial).MemPos== Trial(trial).ProbePos(e));
                Trial(trial).CorrResp(e)= Trial(trial).MemSet(k);
            end
        elseif expinfo.Block==3
            if ismember(Trial(trial).ProbePos(e), Trial(trial).UpdatePos)
                k = find(Trial(trial).UpdatePos== Trial(trial).ProbePos(e));
                m = find(Trial(trial).MemPos == Trial(trial).ProbePos(e));
                if Trial(trial).Operation(k)==1 %Addition
                    Trial(trial).CorrResp(e)= Trial(trial).MemSet(m) + Trial(trial).Update(k);
